...
这也是它与普通 IDE chat 的主要差异：Claude Code 的中心不是回答，而是“在权限边界内持续行动”。
...
推断：
- 内核很可能是一个围绕工具调用和事件流状态机组织的 agent runtime。
- prompt 和上下文编排很可能比公开文档描述的更复杂，但无法从公开资料精确还原。
...
第三方 cleanroom 分析把这层进一步解释为“prompt cache + context packing + query-based pruning”的组合，并指出 Claude Code 会把近期会话、`CLAUDE.md`、相关文件和工具结果拼装成一个动态上下文包。这个方向和官方公开行为是一致的，但具体打包算法没有被官方证实，应视为高可信推断，而不是事实。[Southbridge 分析](https://southbridge.ai/claude-code-an-agentic-cleanroom-analysis)
...
这里一个很关键的工程判断是：Claude Code 的安全模型不是只靠强沙箱，也不是只靠人工审批，而是“审批 + Bash 沙箱 + hooks + setting policy + checkpoint”多层组合。[官方 settings 文档](https://code.claude.com/docs/en/settings) [官方 hooks 文档](https://code.claude.com/docs/en/hooks)
...
## 7. 第三方 cleanroom 分析：哪些值得信，哪些要保留
...
等这三件事成立后，再加 subagent、hooks、插件协议，系统就会开始接近 Claude Code 的“产品味道”。
